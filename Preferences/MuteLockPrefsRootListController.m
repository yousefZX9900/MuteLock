    ...
    if (!_specifiers) {
        ...
        if (logs.count == 0) {
            ...
            for (NSDictionary *entry in logs) {
                NSString *bundleID = entry[@"bundleID"] ?: LOCALIZE(@"UNKNOWN");
...
- (NSArray *)specifiers {
    if (!_specifiers) {
        ...
        // Extreme Protection Settings
        PSSpecifier *dangerGroup = [PSSpecifier groupSpecifierWithName:LOCALIZE(@"DANGER_ZONE")];
        [dangerGroup setProperty:LOCALIZE(@"DANGER_ZONE_FOOTER") forKey:@"footerText"];
        [specs addObject:dangerGroup];

        PSSpecifier *aggrSpec = [PSSpecifier preferenceSpecifierNamed:LOCALIZE(@"EXTREME_PROTECTION") target:self set:@selector(setAggressiveMode:specifier:) get:@selector(readPreferenceValue:) detail:nil cell:PSSwitchCell edit:nil];
        [aggrSpec setProperty:kMuteLockAggressiveMode forKey:@"key"];
        [aggrSpec setProperty:@NO forKey:@"default"];
        [aggrSpec setProperty:@YES forKey:@"isDestructive"];
        [specs addObject:aggrSpec];
...
- (void)setAggressiveMode:(id)value specifier:(PSSpecifier *)specifier {
    if ([value boolValue]) {
        // Display enablement warning.
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:LOCALIZE(@"WARNING") message:LOCALIZE(@"EXTREME_PROTECTION_MSG") preferredStyle:UIAlertControllerStyleAlert];
        ...
        [alert addAction:[UIAlertAction actionWithTitle:LOCALIZE(@"CANCEL") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
            [self reloadSpecifiers]; // Revert preference state on cancellation.
        }]];
        
        [alert addAction:[UIAlertAction actionWithTitle:LOCALIZE(@"TheNuclearOption") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
            [self setPreferenceValue:value specifier:specifier];
            [self respring]; // Trigger immediate respring.
        }]];
        
        [self presentViewController:alert animated:YES completion:nil];
    } else {
        // Display disable warning.
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:LOCALIZE(@"DISABLE_EXTREME_TITLE") message:LOCALIZE(@"DISABLE_EXTREME_MSG") preferredStyle:UIAlertControllerStyleAlert];
        
        [alert addAction:[UIAlertAction actionWithTitle:LOCALIZE(@"CANCEL") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
            [self reloadSpecifiers]; // Revert preference state on cancellation.
        }]];
        
        [alert addAction:[UIAlertAction actionWithTitle:LOCALIZE(@"DISABLE_AND_RESPRING") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            [self setPreferenceValue:value specifier:specifier];
            [self respring]; // Trigger immediate respring.
        }]];
...
- (void)respring {
    ...
    // Attempt 'sbreload' execution.
    int status = posix_spawn(&pid, "/var/jb/usr/bin/sbreload", NULL, NULL, (char* const*)args, NULL);
    
    // Fallback: Terminate backboardd.
    if (status != 0) {
